...
void NetworkManager::begin()
{
  ...
  Serial.print("Network: starting WiFi with hostname ");
...
void NetworkManager::loop()
{
  ...
  if (_pendingReconnect && (now - _pendingReconnectAt) >= 750UL)
  {
  ...
  {
    if ((now - _connectionAttemptAt) >= _connectTimeoutMs)
    {
      _connectionAttemptActive = false;
      _lastReconnectAt = now;
  ...
  if (!_pendingReconnect && (now - _lastReconnectAt) >= _reconnectIntervalMs)
  {
    startStationConnection();
  }
}
...
void NetworkManager::loadSettings()
{
  ...
  // Read-write mode creates the namespace on first boot without NVS NOT_FOUND.
  if (_preferences.begin("wifi_cfg", false))
  {
...
void NetworkManager::startSetupAccessPoint()
{
  ...
  const bool ipConfigured = WiFi.softAPConfig(_setupApIp, _setupApIp, subnet);
